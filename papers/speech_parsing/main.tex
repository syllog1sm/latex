...
repairs. An efficient implementation is available at \verb|<anonymized>|
under an open source license. The future prospects of the system are also quite promising.
...
\caption{\small The algorithm by which transition sequences are selected for
    ...
    modified for partially labelled training data (i.e. there are many possible
    gold sequences).
...
prediction.\footnote{Recall that each training instance
...
using a special dependency label.
...
decoding.  The model is trained following \citet{zhang:cl11}, i.e. we use a
`static' oracle, rather than the dynamic oracle training strategy used for our
Edit transition. The maximum-violation update \citep{huang:12} is used to reduce
the number of training iterations required.
The baseline parser uses the feature set of \citet{zhang:11}, plus our additional
...
All descendants of a word attached to its head by this label are marked as
disfluent.
...
words to the stack
were disfluent predicates, which ofte have subjects and discourse conjunctions as leftward
