...
the parser reaches the configuration $\mathrm{Pass} | \mathrm{the} | \mathrm{red}, \mathrm{rectangle} | \beta, \mathrm{Pass} \rightarrow \mathrm{me}, \empty$. That is, \emph{rectangle}
is at the head of the buffer, with three words on the stack, one (correct) dependency
created, and no words marked disfluent. From this configuration, there are multiple
equally `correct' actions. The Edit transition is correct, because \emph{red}
is disfluent; but both the Left-Arc and the Right-Arc are also correct,
in that there are continuations from them that lead to a fully correct parse
tree and disfluency analysis.
