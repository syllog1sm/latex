...
`-').  We also remove out one-token sentences, as their syntactic
...
\begin{figure}
    ...
    Where \\
    $A' = A \setminus \{x \rightarrow y\;\mathrm{or}\; y \rightarrow x : \forall x \in \{i...j-1\}, \forall y \in \mathbb{N} \}$ \\
...
def train(S, G, k):
  ...
  # A state is finished when its buffer
  # is exhausted and its stack is empty.
  while not is_finished(pred[0][1]) and \
        not is_finished(gold[0][1]):
    pred = extend_beam(pred, k, G, 0)
    gold = extend_beam(gold, k, G, 1)
    p = pred[0]
    # The dynamic oracle determines the
    # number of gold-standard arcs that
    # are unreachable from a state
    cost = dynamic_oracle(p, G)
    if cost >= 1:
      g = gold[0]
      v = p[0] - g[0]
      if v >= max_violn[0]:
        max_violn = (v, p[1], g[1])
  # Update perceptron weights following
  # Collins (2002)
  update_weights(max_violn[1], [2])
...
There are also ten tri-tag templates, for the
...
\textbf{Final joint parse}.  All of the parse accuracy differences were found to
...
child was returned, and the total number of leftward children returned was 1,132.
The most common type of construction were disfluent predicates, where the leftward
children were subjects and discourse conjunctions.  An example of a disfluent
predicate with a fluent subject is shown in Figure \ref{fig:bankrupt}.
