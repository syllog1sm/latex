...
the word Edit marks as disfluent, which means that the parser is not committed
to a mistake if it attaches \emph{the} to \emph{pepper}.  All arcs to and from
disfluent words are deleted, and the fluent parse is scored using the Sparseval
measure.  We also exploit the ease with which the model can incorporate arbitrary
...
repairs. The system is also quite efficient, and has quite promising future prospects.
Because the parser is incremental, it should be well suited
unsegmented text such as the output of speech-recognition systems.
...
The state in Line 5 of Figure \ref{fig:bankrupt} is an example of a state that
...
from them that lead to a fully correct analysis.
...
A dynamic oracle is a function that determines the \emph{cost} of applying an
action to a state, in terms of gold-standard arcs that are newly unreachable.
Our training data is partially labelled
...
\begin{verbatim}
def train(S, G, k):
  pred = priority_queue((0, state(S)))
  gold = priority_queue((0, state(S)))
  max_violn = (0, None, None)
  while not is_finished(pred[0][1]) and \
        not is_finished(gold[0][1]):
    pred = extend_beam(pred, k, G, 0)
    gold = extend_beam(gold, k, G, 1)
    p = pred[0]
    cost = dynamic_oracle(p)
    if cost >= 1:
      g = gold[0]
      v = p[0] - g[0]
      if v >= max_violn[0]:
        max_violn = (v, p[1], g[1])
  update_weights(max_violn)

def extend_beam(beam, k, G, is_gold):
  ext = priority_queue()
  for score, state in beam[:k]:
    for w, action in get_scores(state):
      cost = dyn_oracle(action, state, G)
      if cost == 0 or not is_gold:
        ext.push((score+w, action(state)))
  return ext
\end{verbatim}
\caption{\small The maximum-violation update structured perceptron training
    algorithm \citep{huang:12}, updated to use the \citet{goldberg:12} dynamic
    oracle.  The best predicted and gold-standard candidates are found using
    beam-search, and weight updates are performed over the states where the
    best gold candidate receives its lowest score, compared to the best predicted
    candidate. For simplicity, the path-length normalisation adjustment is not
    shown.\label{fig:train_code}}
\end{figure}

The training algorithm is shown in Figure \ref{fig:train_code}.
For each sentence \verb|S| and its gold-standard analysis \verb|G|,
we maintain two priority queues of
$\langle \mathrm{score}, \mathrm{state} \rangle$ tuples: a prediction
beam \verb|pred| and a gold-standard beam \verb|gold|.
The first beam is used to search for the state sequence with the highest model
score, and the second searches for the sequence with the highest score subject
to the constraint that it be zero-cost, i.e. gold-standard.  In other words, only
actions which can result in the gold-standard analysis are considered (this
check is performed using the dynamic oracle).

As parsing proceeds, we search for the \emph{maximum violation}
\citep{huang:12}: the state where the divergence in score between the top elements
of the beams is largest.
Once parsing terminates, we perform the usual structured perceptron update, with
the sequences leading up to the two maximum violation states.
...
used our own implementation.  Our evaluation script is distributed
in the supplementary materials.  Because we do not need to deal with recognition
...
The baseline \textbf{Beam parser} system refers to a standard arc-eager
transition-based parser with a structured averaged perceptron model and beam-search
decoding.  It uses the feature set of \citet{zhang:11}, plus our additional
dependency label features and the Brown cluster features described in Section
\ref{sec:browns}.  The baseline parser marks tokens as disfluent by attaching
them to the rest of the parse tree with a special dependency label, \textsc{erased}.
All descendants of a word attached to its governor with this label are marked as
disfluent.
...
guage the net effect of disfluencies on our parser's accuracy.
...
\citet{zwarts:11}. The two systems operate very differently, and are the two best
published disfluency detection systems.
...
\begin{table}
    ...
    \begin{tabular}{l|rrr|rr}
        ...
        \hline \hline
Baseline joint parser  &	79.6	&	70.6	&	74.8	&	89.9	&	86.9 \\
...
\hline
    \end{tabular}
    \caption{\small Development results for the joint models.  Disfluencies reduce
        parse accuracy by 1.7\% Unlabelled Attachment Score (\textsc{uas}) for
        the baseline model. Our features and Edit transition reduce the gap to
...
the use of a syntactic language model, which seems to make its output more
syntactically consistent.

The \citet{Johnson04a} system can be made to output the parse assigned to the
top-ranked candidate, so it does not necessarily need a parser as a post-process.
However, conditional parsers are typically more accurate than syntactic
language models, motivating the
\emph{parser}~$\rightarrow$~\emph{disfluency~detector}~$\rightarrow$~\emph{parser}
pipeline we have used in the Johnson et al system.
Despite a disfluency detection disadvantage of 1.8\%, this produced higher parse
accuracies than the Qian and Liu pipeline, which went
\emph{disfluency~detector}~$\rightarrow$~\emph{parser}.  
We take this as good motivation for joint modelling: if both tasks need
the other as a pre-process, then it makes sense to do them at once.  The
success of our joint model, which out-performed both pipelines,
seems to confirm this.

Our joint model's disfluency detection accuracy is slightly higher than
the current state-of-the-art, \citet{qian:13}\footnote{
...
as much training data (as we require syntactic annotation, for which there is
less data).  However, we doubt that the improvement would prove statistically
significant, if we had a way to apply our significance testing regime to the
other disfluency systems.
...
the other two disfluency detectors.  However, our system's disfluency scores
on the test set ranged
from 83.3-84.6 over the 20 samples we collected, so it seems unlikely that
the 0.1\% improvement over the \citet{qian:13} system would prove significant,
if we had more than one sample for it.\footnote{
Note that we do perform significance tests for the \emph{parse accuracy} of the
two pipeline systems, by training our parser with different random seeds ---
but each sample has the same disfluency-detected input.}
...
The parser predicted 2,459 Edit transitions,
...
the same word was returned two or more times\footnote{
A word can be returned to the stack twice by being Left-Arced twice by words later
marked as disfluent.}
    --- i.e. 1,471 tokens were returned
...
\begin{itemize}
    ...
    \item 864 were marked fluent.
        \begin{itemize}
            \itemsep0ex
            \item 699 were ultimately correct
            \item 165 were ultimately incorrect
            \begin{itemize}
                \itemsep0ex
                \item 26 were actually disfluent
                \item 139 were fluent, but received the wrong head.
            \end{itemize}
        \end{itemize}
\end{itemize}
