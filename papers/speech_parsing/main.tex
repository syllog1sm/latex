...
features, and design a set of features that capture the
...
\begin{enumerate}
    ...
    \item How long is the \emph{prefix POS tag match} between \szeroLedge...\szero
          and \nzeroLedge...\nzero?
          %\par In the example above, its value would be 2,
...
miss an aspect of disfluency processing that sequence models naturally capture.
...
labelled attachment score (\textsc{las}).
...
baseline parser, which uses the \citet{zhang:11} feature set plus the Brown
cluster features, scores 1.8\% \textsc{uas} lower than the oracle. 

When we add the features described in Sections 5.2, 5.3 and 5.4, the gap is reduced
to 1.2\% (\textbf{+Features}).  Finally, the improved transition system reduces
the gap further still, to 0.8\% \textsc{uas} (\textbf{+Edit transition}).  We also
tested these features in the Oracle parser, but found they were ineffective on fluent
text.

The \textbf{w/s} column shows the tokens analysed per second for each system,
including disfluencies, with a single thread on a 2.4GHz Intel Xeon.  The additional
...
his code available.} despite having approximately half
...
transition seem to be complementary. The \citeauthor{rasooli:13} system offers
...
On the other hand, our Edit transition may have the advantage for
...
disfluency detectors, and is highly efficient, processing over 550 tokens a second.
